...
class _VulnerabilitaScreenState extends State<VulnerabilitaScreen> {
  ...
  // Voci specifiche per le vulnerabilità
  final List<String> _tipologie = ['Tegola rotta/spostata', 'Guaina lesionata', 'Scossalina ostruita', 'Sigillatura mancante', 'Fessurazione muro', 'Altro'];
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                CasellaTestoVocale(label: 'Note / Consigli', valoreIniziale: notaInserita, onChanged: (val) => notaInserita = val),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                prov.aggiungiFotoVulnerabilita(foto.path, tipologiaSelezionata, notaInserita, giornoSelezionato);
                setState(() { _indiceSelezionato = prov.fotoVulnerabilita.length - 1; }); 
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: list.isEmpty 
        ? const Center(child: Text('Nessuna vulnerabilità inserita.\nPremi il tasto in basso.', textAlign: TextAlign.center))
        : Column(
            children: [
              Expanded(
                flex: 3,
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              (list[_indiceSelezionato]['cancellata'] ?? false)
                                ? IconButton(icon: const Icon(Icons.restore, color: Colors.green, size: 30), onPressed: () => provider.impostaCancellata(_indiceSelezionato, false, provider.fotoVulnerabilita))
                                : IconButton(icon: const Icon(Icons.delete, color: Colors.red, size: 30), onPressed: () => provider.impostaCancellata(_indiceSelezionato, true, provider.fotoVulnerabilita)),
                            ],
                          ),
                        )
                      ],
      ...
      bottomNavigationBar: BottomNavigationBar(
        items: const [
          BottomNavigationBarItem(icon: Icon(Icons.arrow_back), label: '5. Ripristini'),
          BottomNavigationBarItem(icon: Icon(Icons.home), label: 'Home'),
          BottomNavigationBarItem(icon: Icon(Icons.arrow_forward), label: '7. Cause (Note)'),
        ],
        selectedItemColor: Colors.blue,
        unselectedItemColor: Colors.grey,
        onTap: (index) {
          if (index == 0) {
            Navigator.pop(context); // Torna alla pagina 5
          } else if (index == 1) {
