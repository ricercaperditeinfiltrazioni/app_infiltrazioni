...
class RipristinoIniezioniScreen extends StatefulWidget {
  const RipristinoIniezioniScreen({super.key});
  @override
  State<RipristinoIniezioniScreen> createState() => _RipristinoIniezioniScreenState();
}

class _RipristinoIniezioniScreenState extends State<RipristinoIniezioniScreen> {
  final ImagePicker _picker = ImagePicker();
  final List<String> _tipologie = ['1° Insufflazione', '2° Insufflazione', '3° Insufflazione', 'Stuccatura finale', 'Altro'];
  ...
  Future<void> _scattaFoto(List<String> giornatePossibili) async {
    ...
    if (!mounted) return;
    String tipologiaSelezionata = '1° Insufflazione';
    ...
    await showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Dettagli Ripristino'),
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(labelText: 'Fase lavoro'),
                ),
                const SizedBox(height: 16),
                CasellaTestoVocale(label: 'Note', valoreIniziale: notaInserita, onChanged: (val) => notaInserita = val),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                prov.aggiungiFotoRipristino(foto.path, tipologiaSelezionata, notaInserita, giornoSelezionato);
                setState(() { _indiceSelezionato = prov.fotoRipristini.length - 1; }); 
  ...
  Widget build(BuildContext context) {
    ...
    final list = provider.fotoRipristini;
    
    // Calcola quante giornate ci sono in base alla selezione della pagina 1
    int numGiorni = 1;
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('5. Ripristino Iniezioni')),
      body: list.isEmpty 
        ? const Center(child: Text('Nessun ripristino inserito.\nPremi il tasto in basso.', textAlign: TextAlign.center))
        : Column(
            children: [
              Expanded(
                flex: 3,
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              IconButton(icon: Icon(Icons.delete, color: Colors.red), onPressed: () => provider.impostaCancellata(_indiceSelezionato, true, provider.fotoRipristini)),
                            ],
                          ),
                        )
                      ],
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: () => _scattaFoto(giornatePossibili), icon: const Icon(Icons.build), label: const Text('Scatta Foto')),
    );
  }
}
